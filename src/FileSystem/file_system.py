...
from src.FileSystem.shared import generate_sha1, id_from_ip, get_machines, request_append_file, send_file, get_receiver_id_from_file, get_replica_ids, get_server_file_path, get_server_file_metadata, write_server_file_metadata, request_create_file
from src.FileSystem.constants import REPLICATION_FACTOR, INIT_FILE_METADATA, GET, MERGE, APPEND, CREATE, START_MERGE, JOIN, MULTIAPPEND_REQUEST
...
def merge_file(file_name):
    ...
    for i, s in enumerate(sockets):
        ...
        while(1):
            ...
            chunk_length = int.from_bytes(byte_len, byteorder="little")
            read_bytes = 0

            while (1):  # receive memtable data
                new_content = s.recv(min(chunk_length - read_bytes, 10240))
